...
- **Python 3.12** is strongly recommended. **Windows:** **`scripts/setup_venv.ps1`** prefers **`py -3.12`**. **macOS / Linux:** **`scripts/setup_venv.sh`** prefers **`python3.12`** when installed. On **Python 3.14**, official **CUDA wheels for PyTorch are often absent** on Windows/Linux NVIDIA setups, so `pip` may install **`torch` CPU-only** — the UI works but the LLM stays on CPU.
- **GPU:** **NVIDIA + CUDA** on Windows/Linux (VRAM e.g. **8 GB**); **Apple Silicon** can use **MPS** when PyTorch supports it (see **`check_gpu`** scripts below).
- **Windows + NVIDIA:** If **`torch.version.cuda`** is **`None`**, inference uses **CPU** (much slower). **`setup_venv.ps1`** reinstalls **`torch`** from the **PyTorch CUDA 12.4** index after `requirements.txt`. **`setup_venv.sh` does not install CUDA wheels** — use [pytorch.org](https://pytorch.org/get-started/locally/) for Linux NVIDIA or rely on **`requirements.txt`** on Mac (**MPS**, not CUDA).

### Optional: verify the GPU / MPS
...
You should see **`cuda_available True`** when CUDA PyTorch is installed (often build tag **`+cu124`**).
...
On **Apple Silicon**, check **`mps_available True`** for Metal acceleration where supported.
...
| `scripts/setup_venv.ps1` | **Windows:** Creates **`.venv`** (prefers Python **3.12**), installs deps, then **`torch`** with **CUDA 12.4** from PyTorch’s index. |
| `scripts/setup_venv.sh` | **macOS / Linux:** Creates **`.venv`**, **`pip install -r requirements.txt`** (no CUDA reinstall — use PyTorch docs if you need a specific **`torch`** build). Writes **`env_local.sh`** once for HF cache under **`./.cache/`**. |
| `scripts/check_gpu.ps1` | **Windows:** Quick **`torch` + CUDA** check. |
| `scripts/check_gpu.sh` | **macOS / Linux:** Prints **`torch`** version, **`cuda_available`**, **`mps_available`**. |
| `scripts/run_app.ps1` / `run_web.ps1` / `run_vault_graph.ps1` | **Windows** wrappers: **`app.py`**, **`uvicorn`** on **8000** / **8001**. |
| `scripts/run_app.sh` / `run_web.sh` / `run_vault_graph.sh` | **macOS / Linux** — same roles (sources **`env_local.sh`** if present). |
...
```bash
cd /path/to/ai_sample
./scripts/setup_venv.sh
```

This:

- Creates **`.venv`** using **`python3.12`** if available, otherwise **`python3`**.
- Runs **`pip install -r requirements.txt`** only (no CUDA wheel swap — correct for **Mac** and many **Linux CPU** setups).
- Creates **`env_local.sh`** once so Hugging Face caches live under **`./.cache/`**.

Optional before manual commands:

```bash
source ./env_local.sh
```

**Manual venv (any OS):** `python -m venv .venv`, activate, then `pip install -r requirements.txt`. On **Windows + NVIDIA**, prefer **`setup_venv.ps1`** or install **CUDA `torch`** per [pytorch.org](https://pytorch.org/get-started/locally/).
...
```bash
./scripts/run_app.sh --reindex
./scripts/run_app.sh
```

**Any OS (activated venv):**
...
Extra arguments are forwarded to **uvicorn**, e.g. **`./scripts/run_web.sh --reload`**.
...
The header toolbar includes **New chat** (clears the UI and starts a fresh session id), **Upload .md** (writes files into the vault folder), **Reindex** (rebuilds Chroma and reloads the vector store — status text shows chunk/file counts and timestamp), and **Vault files** (expandable list of **`*.md`** paths under the vault).

**Chat persistence:** messages are stored in **SQLite** (default **`.rag_chat.sqlite`** next to **`app.py`**; WAL mode). Override with **`chat.db_path`** in **`config.yaml`** or env **`RAG_CHAT_DB`** (absolute path). The UI saves **`session_id`** in **`localStorage`** and reloads history on refresh. **`POST /api/rag/stream`** accepts optional **`session_id`**; the first SSE event is **`type: meta`** with the resolved id. Recent turns are injected into the prompt for follow-ups; retrieval uses the last few exchanges plus the new question so short replies (“expand that”) still hit the vault. Tune **`chat.*`** in **`config.yaml`**.

**Obsidian-native extras (web UI & API):** optional **`filter_note`** / **`filter_tag`** restrict retrieval (single vault path or tag substring). **`expand_wikilinks`** (default on) adds **one-hop** chunks from notes linked by **`[[wikilinks]]`** to/from the primary hits. SSE **`sources`** include **Chroma distance** (lower = closer match), **`obsidian_uri`** (set **`obsidian.vault_name`** for `obsidian://open?vault=…&file=…`, else absolute **`path=`**). Checkbox **Suggest vault edit** runs a **second short generation** and may emit **`type: vault_edit`** (JSON path + markdown patch). **Export chat** writes **`vault/Chat exports/<date>-chat-<id>.md`**. **`POST /api/chat/export`** with **`session_id`** returns **`vault_rel_path`**.

| Path | Purpose |
|------|---------|
| `/rag/chat` | Main UI (**`index.html`**) — Chroma + LLM + graph. |
| `/vault/graph`, `/graph` | Redirect to **`/rag/chat`**. |
| `POST /api/rag/stream` | SSE chat (alias: **`/api/chat/stream`**). JSON: **`message`**, **`session_id`**, **`show_sources`**, **`filter_note`**, **`filter_tag`**, **`expand_wikilinks`**, **`suggest_vault_edit`**. Events: **`meta`**, **`sources`** (with **`distance`**, **`obsidian_uri`**), **`token`**, optional **`vault_edit`**, **`done`**. |
| `GET /api/chat/history` | Query **`session_id`** — list stored messages for that session. |
| `GET /api/chat/sessions` | Recent sessions (metadata). |
| `DELETE /api/chat/sessions/{id}` | Delete a session and its messages. |
| `POST /api/chat/export` | JSON **`{ "session_id" }`** — writes **`Chat exports/…-chat-….md`** into the vault. |
...
| **Always CPU / very slow** | **Windows:** Python **3.12** + rerun **`setup_venv.ps1`**; **`check_gpu.ps1`**. **Mac:** run **`./scripts/check_gpu.sh`** — **MPS** depends on PyTorch build; install **`torch`** per [pytorch.org](https://pytorch.org/get-started/locally/). **Linux NVIDIA:** install CUDA **`torch`** the same way. |
